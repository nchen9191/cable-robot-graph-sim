...
from utilities.loss_functions import CauchyLoss
...
def train():
    ...
    eval_steps = [25, 20, 10, 5, 5, 3]
    dt_deltas = [0.0, 0.0, 0.03, 0.03, 0.04, 0.05]
    vel_min_dts = [0.0, 0.0, 0.05, 0.05, 0.05, 0.05]

    params = list(
        zip(target_dts, epochs, learning_rates, load_sim, batch_sizes, eval_steps, mix_ratios, dt_deltas, vel_min_dts))
    for n, e, lr, load, batch_size, eval_step, mix_r, dt_d, vm_dt in params[3:]:
        ...
        trainer = RealTensegrity5DRecurrentMotorGNNTrainingEngine(config_file,
                                                                  torch.nn.MSELoss(),
                                                                  0.01,
                                                                  logger)
